...
let controllers, responseHandlers, services, controllerCache;

beforeAll(async () => {
  controllers = await import('../../src/apis/bankAccounts/bankaccountController.js');
  ...
  // Guarantee all mocks are Jest mock functions
...
describe('bankaccountController', () => {
  ...
  describe('getBankaccount', () => {
    let req, res;
    beforeEach(() => {
      req = { user: { company_id: 1, role: 'ADMIN', user_id: 2, designation: 'ADMIN' }, query: {} };
      res = {};
    });
    it('should return cached data if available', async () => {
      controllerCache.readJsonCache.mockResolvedValue({ cached: true });
      controllerCache.shouldServeCachedResponse.mockReturnValue(true);
      responseHandlers.sendSuccess.mockImplementation((res, data, msg) => { res._sent = { data, msg }; return res; });
    ...
    it('should call service and cache if no cache', async () => {
      ...
      services.getBankaccountService.mockResolvedValue([{ id: 1 }]);
      controllerCache.writeJsonCache.mockResolvedValue();
      responseHandlers.sendSuccess.mockImplementation((res, data, msg) => { res._sent = { data, msg }; return res; });
      await controllers.getBankaccount(req, res);
      expect(services.getBankaccountService).toHaveBeenCalled();
