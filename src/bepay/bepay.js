...
import { getBepayDetailsByCompanyIdDao } from '../apis/company/companyDao.js';
...
/**
 * Initiate a payout request to Bepay API.
...
const baseUrl = config.bepay.baseUrl;
const initiatePayoutUrl = config.bepay.initiatePayout;
const walletBalanceUrl = config.bepay.walletBalance;

export async function initiateBepayPayout(payload, company_id) {
  const newPayload = {
  ...
  try {
    const clickrrWalletBalance = await getBepayWalletBalance({ company_id });
    ...
    const clickrrDetails = await getBepayDetailsByCompanyIdDao(company_id);
    ...
    logger.log('Bepay payout initiated successfully:', {
...
export async function getBepayWalletBalance(reqOrParams, res) {
  try {
    ...
    const bepayDetails = await getBepayDetailsByCompanyIdDao(company_id);
    const apiKey = bepayDetails.api_key;
    const apiSecret = bepayDetails.api_secret;
    ...
    const successMsg = 'Bepay wallet balance fetched successfully';
    if (isExpress) {
    ...
    logger.error(
      'Error fetching Bepay payout status:',
      error.response?.data || error.message || error,
    );
    throw error;
  }
...
export async function createBepayPayout(
  payload,
  ids,
  singleWithdrawData,
  bankId,
  ...
  try {
    ...
    if (payload.txnStatus) {
      ...
      checkBepay = await initiateBepayPayout(
        singleWithdrawData,
        ids.company_id,
      );
    }

    const status = checkBepay.txnStatus;
    ...
    if (!status) {
      ...
      payload.utr_id = checkBepay?.utr || '';
      payload.approved_at = new Date().toISOString();
    } else if (status === 'Failed' || status === 'failed') {
      payload.status = Status.REJECTED;
      payload.rejected_reason = checkBepay?.message || 'Transaction failed';
      payload.rejected_at = new Date().toISOString();
    } else {
      payload.status = Status.PENDING;
    }
    ...
    logger.info('Bepay payout processed successfully:', payload);
    return payload;
  } catch (error) {
    payload.status = Status.REJECTED;
    payload.bank_acc_id = bankId;
    ...
    logger.error('Bepay payout error:', error.message);
    logger.warn('Bepay payout error response', payload);
    return payload;
  }
}
