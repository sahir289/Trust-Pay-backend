...
export const createTelegramSender = () => {
    ...
    return async (chatId, message, replyToMessageId, token = config?.telegramBotToken) => {
      if (!token) {
        throw new Error('TELEGRAM_BOT_TOKEN is required either via argument or config.');
      }
      ...
      try {
        ...
        logger.error('Error sending message to Telegram:', error?.message || 'Request failed with status code 429');
