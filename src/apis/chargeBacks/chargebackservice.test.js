...
// Remove this invalid import: import { trackVendorsNetBalance } from './chargeBackService.js';
...
// Add this: Mock the trackVendorsNetBalance utility to prevent real execution
jest.mock('../../utils/trackVendorsNetBalance.js', () => ({
  trackVendorsNetBalance: jest.fn(),
}));
const trackVendorsNetBalance = require('../../utils/trackVendorsNetBalance.js').trackVendorsNetBalance;
...
describe('ChargeBack Service', () => {
  ...
  beforeEach(() => {
    ...
    // Mock trackVendorsNetBalance to resolve without errors (void return)
  ...
  describe('createChargeBackService', () => {
    ...
    beforeEach(() => {
      ...
      getCalculationforCronDao.mockResolvedValue([{ id: 10, user_id: 1 }]); // Add user_id for trackVendorsNetBalance call
      updateCompanyConfigDao.mockResolvedValue();
      updateMerchantDao.mockResolvedValue();
      updateCalculationBalanceDao.mockResolvedValue({ rows: [{}] }); // Mock response for trackVendorsNetBalance
    });
    ...
    test('should create a chargeback and commit transaction', async () => {
      ...
      expect(trackVendorsNetBalance).toHaveBeenCalledWith(1, mockConn, { rows: [{}] }); // Verify call with vendor user_id and response
  ...
  describe('updateChargeBackService', () => {
    beforeEach(() => {
      getChargebackByIdDao.mockResolvedValue([
        { id: 1, created_at: new Date().toISOString(), amount: 50, vendor_user_id: 1 }
      ]);
      updateChargeBackDao.mockResolvedValue({ merchant_user_id: 2, vendor_user_id: 1, amount: 100 });
      getCalculationforCronDao.mockResolvedValue([{ id: 1, user_id: 1 }]);
      updateCalculationBalanceDao.mockResolvedValue({ rows: [{}] });
    });
    ...
    test('should update chargeback successfully', async () => {
      ...
      expect(trackVendorsNetBalance).toHaveBeenCalledWith(1, mockConn, { rows: [{}] });
  ...
  describe('getChargeBacksService pagination edge cases', () => {
    ...
    test('should expand merchant_user_id with hierarchy siblings', async () => {
      ...
      const filters = { merchant_user_id: [123] }; // Pre-set to test expansion
      await getChargeBacksService(filters, Role.MERCHANT, 1, 10, 123);
      expect(filters.merchant_user_id).toEqual(expect.arrayContaining([123, 99])); // Use arrayContaining for modified array
    });
  });
  ...
  describe('updateChargeBackService amount change scenarios', () => {
    ...
    test('should handle increase in amount', async () => {
      getChargebackByIdDao.mockResolvedValue([{ id: 1, created_at: new Date().toISOString(), amount: 50, vendor_user_id: 1 }]);
      updateChargeBackDao.mockResolvedValue({ merchant_user_id: 2, vendor_user_id: 1, amount: 100 });
      const result = await updateChargeBackService({ id: 1, company_id: 1 }, { amount: 100 });
      expect(updateChargeBackDao).toHaveBeenCalled();
      // Code sets total_chargeback_count: 1, total_chargeback_amount: 50 (delta), current_balance: -50, net_balance: -50
      expect(updateCalculationBalanceDao).toHaveBeenCalledWith(
        { id: 1 },
        {
          total_chargeback_count: 1,
          total_chargeback_amount: 50,
          current_balance: -50,
          net_balance: -50,
        },
        mockConn
    ...
    test('should handle decrease in amount', async () => {
      getChargebackByIdDao.mockResolvedValue([{ id: 1, created_at: new Date().toISOString(), amount: 150, vendor_user_id: 1 }]);
      updateChargeBackDao.mockResolvedValue({ merchant_user_id: 2, vendor_user_id: 1, amount: 100 });
      const result = await updateChargeBackService({ id: 1, company_id: 1 }, { amount: 100 });
      expect(updateChargeBackDao).toHaveBeenCalled();
      // Code sets total_chargeback_count: 1, total_chargeback_amount: -50 (delta), current_balance: 50 (since -(-50)), net_balance: 50
      expect(updateCalculationBalanceDao).toHaveBeenCalledWith(
        { id: 1 },
        {
          total_chargeback_count: 1,
  ...
  // REMOVE THIS ENTIRE BLOCK - it's invalid and misplaced
