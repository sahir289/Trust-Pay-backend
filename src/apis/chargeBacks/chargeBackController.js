...
const updateChargeBack = async (req, res) => {
        ...
        if (bodyError) {
            return ValidationError(res, bodyError.details[0].message, 'Validation Error');
        }
...
const deleteChargeBack = async (req, res) => {
        ...
        const { id } = req.params;  
        const {company_id,role, user_id} = req.user;
        await transactionWrapper(deleteChargeBackService)({id,company_id},{updated_by : user_id,  is_obsolete: true }, role);
