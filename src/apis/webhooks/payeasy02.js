 ...
 */
function decryptPayEasy02Data(encryptionKey, encryptedObj) {
...
export const payEasy02Webhook = async (req, res) => {
  logger.info('payEasy02Webhook called', req.body);
  let utr = null;
  ...
  try {
    ...
    if (!iv || !encryptedData) {
      logger.error('Missing iv or encryptedData in payEasy02 webhook');
      return;
    }

    const encryptionKey = config.payeasy02?.encryptionKey;
    const body = decryptPayEasy02Data(encryptionKey, { iv, encryptedData });

    logger.info('Decrypted payEasy02 webhook body:', body);

    const merchantOrderId = body?.orderId;
    utr = body?.utr;
    
    const lockAcquired = await acquireLock(utr, 'payEasy02');
    if (!lockAcquired) {
    ...
    logger.info(
      `[PayEasy02] Fetching PayIn for merchantOrderId: ${merchantOrderId}`,
    );
    const payIn = await getPayInIntentDao(merchantOrderId);

    if (!payIn) {
      logger.error(
        `[PayEasy02] PayIn not found for merchantOrderId: ${merchantOrderId}`,
      );
      return;
    }

    logger.info(`[PayEasy02] PayIn fetched:`, {
      merchantOrderId,
      status: payIn.status,
    ...
    logger.info(
      `[PayEasy02] Checking for existing UTR: ${payload.userSubmittedUtr}`,
    );
    ...
    if (utrAlreadyExist) {
      logger.warn(
        'Duplicate UTR received in payEasy02 webhook:',
        payload.userSubmittedUtr,
      );
      return;
    }

    logger.info(`[PayEasy02] UTR check passed, proceeding with processing`);

    if (isSuccess) {
      logger.info(
        `[PayEasy02] Creating bank response for approved transaction`,
      );
    ...
    logger.error('[PayEasy02] Webhook processing error:', {
    ...
    if (utr) {
      await releaseLock(utr, 'payEasy02');
    }
  }
};
