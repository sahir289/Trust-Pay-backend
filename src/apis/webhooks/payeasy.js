 ...
 */
function decryptPayEasyData(encryptionKey, encryptedObj) {
...
export const payEasyWebhook = async (req, res) => {
  logger.info('payEasyWebhook called', req.body);
  let utr = null;
  ...
  try {
    ...
    if (!iv || !encryptedData) {
      logger.error('Missing iv or encryptedData in payEasy webhook');
      return;
    }

    const encryptionKey = config.payeasy?.encryptionKey;
    const body = decryptPayEasyData(encryptionKey, { iv, encryptedData });
    
    logger.info('Decrypted payEasy webhook body:', body);
    ...
    if (utrAlreadyExist) {
      logger.warn(
        'Duplicate UTR received in payEasy webhook:',
        payload.userSubmittedUtr,
      );
      return;
    }
