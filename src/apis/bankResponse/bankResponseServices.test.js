...
it('should calculate parent commission for sub-vendor on bank response creation', async () => {
  const payload = '1000.00 nil utr123 bank_1 true';
  const companyId = '123';
  const role = 'MERCHANT';
  const name = 'test_user';
  ...
  const subVendor = { id: 'sub_vendor_1', balance: 10000, payin_commission: 0.01, user_id: 'sub_user_1', code: 'SUB123', designation_name: Role.SUB_VENDOR, designation: Role.SUB_VENDOR, config: { is_owned: false } };
  getVendorsBankReponseDao.mockResolvedValue([subVendor]);

  const parentVendor = { id: 'parent_vendor_1', payin_commission: 0.02, user_id: 'parent_user_1' };
  const mockUserHierarchy = [{ config: { parent: 'parent_user_1' } }];
  
  // Destructure the mocked function (now guaranteed to be jest.fn() due to top-level mock)
  ...
  getVendorsDao.mockResolvedValue([parentVendor]); // For parent fetch

  getBankaccountDashBoardReportDao.mockResolvedValue([
    { balance: 5000, today_balance: 2000, payin_count: 1, user_id: 'sub_user_1', config: {}, nick_name: 'Bank A' },
  ]);

  getCalculationforCronDao.mockResolvedValue([{ id: 'calc_1', created_at: new Date('2025-08-18'), current_balance: 0, net_balance: 0, total_payin_amount: 0, total_payin_commission: 0, total_payin_count: 0 }]);
  updateBankaccountDao.mockResolvedValue({ today_balance: 3000 });
  updateVendorDao.mockResolvedValue({});
  updateCalculationBalanceDao.mockResolvedValue({});
  ...
  expect(getUserHierarchysDao).toHaveBeenCalledWith({ user_id: 'sub_user_1' });
  ...
  expect(result).toEqual({ message: 'Entry created successfully', data: expect.any(Object) });
    ...
    it('should handle dispute when amounts mismatch', async () => {
      const payload = '1200.00 nil utr123 bank_1 true';
      const companyId = '123';
      const role = 'MERCHANT';
      const name = 'test_user';
      ...
      updatePayInUrlDao.mockResolvedValue({
        ...
        merchant_order_id: 'order_1',
        amount: 1000,
        payin_merchant_commission: 24, // 1200 * 0.02
        payin_vendor_commission: 12, // 1200 * 0.01
        config: { urls: { notify: 'url' } },
      });
      ...
      expect(updatePayInUrlDao).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          duration: '00:05:00',
          is_notified: true,
          payin_merchant_commission: 0.24,
          payin_vendor_commission: 0.12,
          status: Status.DISPUTE,
          user_submitted_utr: 'utr123',
        }),
        expect.any(Object),
        expect.objectContaining({ utr: 'utr123', amount: 1200 })
      );
      expect(result).toEqual({
        message: 'Entry is in Dispute with order_1',
      });
    });
  });
  ...
  describe('createBankResponseWebHookService', () => {
    ...
    it('should calculate parent commission for sub-vendor in webhook creation', async () => {
      const payload = '1000.00 nil utr123 bank_1 true';
      const companyId = '123';
      const role = 'MERCHANT';
      const name = 'test_user';
      ...
      const parentVendor = { id: 'parent_vendor_1', payin_commission: 0.02, user_id: 'parent_user_1' };
      const mockUserHierarchy = [{ config: { parent: 'parent_user_1' } }];
      const getUserHierarchysDaoMock = require('../userHierarchy/userHierarchyDao').getUserHierarchysDao;
      getUserHierarchysDaoMock.mockResolvedValue(mockUserHierarchy);
      getVendorsDao.mockResolvedValue([parentVendor]);
      ...
      expect(getUserHierarchysDaoMock).toHaveBeenCalledWith({ user_id: 'sub_user_1' });
      expect(updateCalculationBalanceDao).toHaveBeenCalledWith(
        { id: 'calc_1' },
        expect.objectContaining({
          current_balance: 999.9,
          net_balance: 999.9,
  ...
  describe('resetBankResponseService', () => {
    beforeEach(() => {
      const fixedDate = new Date('2025-08-18T10:00:00.000Z');
    ...
    it('should throw NotFoundError if new UTR already exists', async () => {
      const id = '1';
      const userData = {
        company_id: '123',
        user_name: 'test_user',
      ...
      getBankResponseDao.mockResolvedValue({ id: '1', utr: 'old_utr', amount: 1000, bank_id: 'bank_1', config: {} });
      getPayInsForResetBankResDao.mockResolvedValue([{ status: 'PENDING' }]);
      getBankResponseDao.mockResolvedValue({ id: 'existing' }); // Existing new UTR

      await expect(resetBankResponseService(mockConnection, id, userData)).rejects.toThrow('UTR is already confirmed with Merchant Order ID order_1. No changes applied. Previous Amount: undefined');
    });

    it('should throw BadRequestError if new bank ID is same as old', async () => {
      const id = '1';
      const userData = {
        company_id: '123',
        user_name: 'test_user',
        ...
        amount: undefined,
        utr: undefined,
        bank_id: 'bank_1', // Same as existing
      };
      getBankResponseDao.mockResolvedValue({ id: '1', utr: 'utr123', amount: 1000, bank_id: 'bank_1', config: {} });
      getPayInsForResetBankResDao.mockResolvedValue([{ status: 'PENDING' }]);
      getBankaccountDao.mockResolvedValue([{ id: 'bank_1', nick_name: 'Bank A' }]);

      await expect(resetBankResponseService(mockConnection, id, userData)).rejects.toThrow('Please provide a different bank account ID');
    });
  });
