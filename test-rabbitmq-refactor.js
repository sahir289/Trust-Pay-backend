 ...
 * Run with this command: node test-rabbitmq-refactor.js
 */
...
test('Connection Manager Module Loads', async () => {
  ...
  if (!module.RabbitMQConnection)
    throw new Error('RabbitMQConnection not exported');
  if (!module.publisherConnection)
    throw new Error('publisherConnection not exported');
  if (!module.consumerConnection)
    throw new Error('consumerConnection not exported');
});

// Test 2: Publisher Module Loads
test('Publisher Module Loads', async () => {
  const module = await import('./src/utils/rabbitmq-publisher.js');
  if (!module.publishBankResponse)
    throw new Error('publishBankResponse not exported');
  if (!module.publishBankResponseBulk)
    throw new Error('publishBankResponseBulk not exported');
...
test('Worker Module Loads', async () => {
  ...
  if (!module.startBankResponseWorker)
    throw new Error('startBankResponseWorker not exported');
  if (!module.startBankResponseHandler)
    throw new Error('startBankResponseHandler not exported');
...
test('Backward Compatibility (old module works)', async () => {
  ...
  if (!module.publishBankResponse)
    throw new Error('publishBankResponse not re-exported');
  if (!module.publishBankResponseBulk)
    throw new Error('publishBankResponseBulk not re-exported');
});
...
test('Configuration Has Required Fields', async () => {
  ...
  if (!rabbitmq.bankResponseQueue)
    throw new Error('rabbitmq.bankResponseQueue not configured');
  if (rabbitmq.prefetchCount === undefined)
    throw new Error('rabbitmq.prefetchCount not configured');
  if (!rabbitmq.retryAttempts)
    throw new Error('rabbitmq.retryAttempts not configured');
});

// Test 6: Connection Manager Class
test('Connection Manager Class Works', async () => {
  const { RabbitMQConnection } = await import(
    './src/utils/rabbitmq-connection.js'
  );
  const connection = new RabbitMQConnection('test');

  if (typeof connection.connect !== 'function')
    throw new Error('connect method missing');
  if (typeof connection.getChannel !== 'function')
    throw new Error('getChannel method missing');
...
test('Worker Has Retry Logic', async () => {
  ...
  const workerContent = fs.readFileSync(
    './src/worker/bank-response-worker.js',
    'utf-8',
  );
...
test('Publisher Has Database Fallback', async () => {
  const fs = await import('fs');
  const publisherContent = fs.readFileSync(
    './src/utils/rabbitmq-publisher.js',
    'utf-8',
  );
...
test('Publisher and Consumer Use Separate Connections', async () => {
  const { publisherConnection, consumerConnection } = await import(
    './src/utils/rabbitmq-connection.js'
  );
  ...
  if (
    publisherConnection.connectionName === consumerConnection.connectionName
  ) {
...
test('Connection Manager Has Exponential Backoff', async () => {
  const fs = await import('fs');
  const connectionContent = fs.readFileSync(
    './src/utils/rabbitmq-connection.js',
    'utf-8',
  );
